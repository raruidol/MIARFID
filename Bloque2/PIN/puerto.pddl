...
(:types muelle numero cinta base grua - object
  ...
  (:predicates
	...
	(:action stack-container
		...
		:precondition ( and (sujetando ?g ?c1) (tope ?c2 ?p) (esta_en ?p ?m) (esta_en ?g ?m) (not (es_objetivo ?c1)) (altura ?n ?p) (not (max_alt ?n ?m)) (sucesor ?n ?n1))
	...
	(:action objectivestack-container
		:parameters (?g - grua ?c1 - contenedor ?c2 - base ?p - pallet ?m - muelle ?n - numero ?n1 - numero)
		:precondition ( and (sujetando ?g ?c1) (tope ?c2 ?p) (esta_en ?p ?m) (esta_en ?g ?m) (es_objetivo ?c1) (altura ?n ?p) (not (max_alt ?n ?m)) (sucesor ?n ?n1))
	...
	(:action unstack-container
		:parameters (?g - grua ?c1 - contenedor ?b - base ?p - pallet ?m - muelle ?n - numero ?n1 - numero)
		:precondition ( and (libre ?g) (tope ?c1 ?p) (esta_en ?p ?m) (esta_en ?c1 ?m) (esta_en ?g ?m) (sucesor ?n1 ?n2) (altura ?n2 ?p) (encima ?c1 ?b))
		:effect ( and (not (libre ?g)) (not (tope ?c1 ?p)) (tope ?b ?p) (not (esta_en ?c1 ?m)) (altura ?n1 ?p) (not (encima ?c1 ?b)) (sujetando ?g ?c1) (not (disponible ?c1)) (disponible ?b) )
	)
